-- Create engagement_types table if it doesn't exist
...
-- Insert all engagement types (with ON CONFLICT to handle duplicates)
...
-- Update the engagement table to include engagement_type_id if it doesn't exist
ALTER TABLE engagement 
ADD COLUMN IF NOT EXISTS engagement_type_id UUID REFERENCES engagement_types(id);

-- Display results
SELECT 'Engagement Types Created:' as status, COUNT(*) as count FROM engagement_types;
SELECT name, default_duration_hours FROM engagement_types ORDER BY sort_order LIMIT 10;