-- Complete KanbanPM Database Schema (Fixed for existing table structure)
-- Missing Tables to Create in Supabase

-- =============================================
-- 1. USERS TABLE (for authentication and login)
-- Already exists based on your migrations, but create if missing
-- =============================================
...
-- Insert all your engagement types (only if table is empty)
...
) AS new_types(name, description, default_duration_hours, sort_order)
WHERE NOT EXISTS (SELECT 1 FROM engagement_types LIMIT 1);
...
) AS new_settings(setting_key, setting_value, setting_type, description, is_public)
...
-- Ensure user_role_assignments has user_id column (should already exist)
...
-- Engagement types indexes
CREATE INDEX IF NOT EXISTS idx_engagement_types_active ON engagement_types(is_active);
CREATE INDEX IF NOT EXISTS idx_engagement_types_sort ON engagement_types(sort_order);
...
-- Apply update triggers to all new tables with updated_at
...
-- =============================================
-- ROW LEVEL SECURITY (REMOVED PROBLEMATIC POLICIES)
-- =============================================
...
-- Simple policies that don't reference complex joins
-- Users can only see their own data
CREATE POLICY users_own_data ON users
    FOR ALL USING (auth.uid() = id);

-- All authenticated users can read audit logs (simplified)
CREATE POLICY audit_logs_read ON audit_logs
    FOR SELECT USING (auth.uid() IS NOT NULL);

-- Users can see time entries they created or for engagements they own
